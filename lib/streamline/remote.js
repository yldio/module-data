...
function remote (mdlName, options, done) {
  ...
  if (options.versions && !Array.isArray(options.versions)) {
    return done(new Error('Versions specified on options should be an array'))
  }

  if (!options.lazy && !options.versions) {
    return done(new Error('When running module-data:remote non-lazy you should specify the versions on the options.'))
  }
  ...
  return async.series({
    ...
    for (var v in versions) {
      if (!options.lazy && !~options.versions.indexOf(v)) {
        continue
      }
