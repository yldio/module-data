...
SlocApi.prototype.messageHandler = function (worker, message) {
  if (!message) {
    message = worker
  }
...
SlocApi.countReal = function countRecursively (obj, cursor, slocCursor, isBottom) {
  ...
  if (isBottom) {
    ...
    if (cursor.length < 1) {
      childrenKeys = Object.keys(obj.dependencies)
    ...
    if (!closestChildWithGrandChilds) {
      ...
      childrenKeys.forEach(function (dep) {
        slocCursor.push({
          sloc: obj.dependencies[dep].sloc,
          children: {}
        })
      })
