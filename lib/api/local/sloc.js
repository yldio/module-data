...
function SlocApi (errRef) {
  ...
  var checkWorkersHealth = setTimeout(function () {
    cluster.disconnect(function () {
      errRef = new Error('Something went wrong with forking the workers required.')
    })
  }, 2000)
      ...
      cluster.on('disconnect', function (worker) {
        ...
        if (flag) {
          ...
          if (exitWorkerThreshold < 0) {
            errRef = new Error('Something is killing the workers.')
            return
          }
...
SlocApi.prototype.messageHandler = function (message) {
  if (this.work[message.id]) {
    var localData = this.work[message.id]
